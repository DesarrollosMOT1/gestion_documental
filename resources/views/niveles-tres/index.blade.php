    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-sm-12">
                    ...
                    <div class="card-body bg-white">
                        <div class="table-responsive">
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
                                    @foreach ($nivelesTres as $nivelesTre)
                                        <tr>
                                            <td>{{ ++$i }}</td>
										...
										<td >{{ $nivelesTre->id_referencias_gastos }}</td>
                                            ...
                                            <td>
                                                <form action="{{ route('niveles-tres.destroy', $nivelesTre->id) }}" method="POST">
                                                    <a class="btn btn-sm btn-primary " href="{{ route('niveles-tres.show', $nivelesTre->id) }}"><i class="fa fa-fw fa-eye"></i> {{ __('Show') }}</a>
                                                    <a class="btn btn-sm btn-success" href="{{ route('niveles-tres.edit', $nivelesTre->id) }}"><i class="fa fa-fw fa-edit"></i> {{ __('Edit') }}</a>
                                                    @csrf
                                                    @method('DELETE')
                ...
                {!! $nivelesTres->withQueryString()->links() !!}
            </div>
        </div>
    </div>
@endsection
